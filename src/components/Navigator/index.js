...
import { SET_NAVIGATOR_EXPAND_POSTS_LIST, SET_CATEGORY_FILTER_SHOW } from '@/store'

export default ({ posts, ...props }) => {
  const dispatch = useDispatch()
  const navigatorPostsIsOpen = useSelector(state => state.navigatorPostsIsOpen)
  const navigatorExpandPostsList = useSelector(state => state.navigatorExpandPostsList)
  ...
  return asiderShow ? (
    <aside className={classes}>
      ...
      <NavigatorPosts display={navigatorPostsIsOpen} showPostsList={navigatorExpandPostsList} callback={handleToggle} posts={posts} />
    </aside>
  ) : null
}
