...
- 生成随机验证码
  ...
  function createCode() {
    ...
    for (var i = 0; i < codeLength; i++) {
      ...
      var index = Math.floor(Math.random() * 36); /* 通过随机数生成索引（0~35） */
      code += random[index]; /* 根据索引取得随机数加到code上,循环 `codeLength=4`次，取到`codeLength=4` 随机字符 */
    }
    return code;
  }
  ```
...
- `firefox` 下实现的方法
  ...
  ```html
  <a href="#" onClick="javascript:window.sidebar.addPanel('百度一下','http://www.baidu.com','');">收藏本站</a>
  ```
